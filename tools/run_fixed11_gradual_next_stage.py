...
from quant_lab.research.optimized_v2_grid import load_frozen_targets
...
def candidate_run_hash(
    *, candidate: ExperimentCandidate, source_target_hash: str, start: str, end: str,
    initial_cash: float, costs: CostModel, schema_version: str = SCHEMA_VERSION,
) -> str:
    return _sha256_json({
...
def should_resume(record_or_path: Mapping[str, Any] | str | Path, candidate_hash: str) -> bool:
    if isinstance(record_or_path, Mapping):
        ...
        try:
            ...
            return False
        complete = all((run_dir / name).is_file() for name in REQUIRED_CANDIDATE_ARTIFACTS)
    try:
...
def build_route_candidates(
    ...
    return_cores = list(ranked_return_cores or core_pool[:3])[:3]
    defensive_cores = list(ranked_defensive_cores or core_pool[:2])[:2]
...
def rank_core_variants(scores: pd.DataFrame) -> tuple[list[Any], list[Any], pd.DataFrame]:
    audited = scores.loc[
        scores["account_reconciliation_error"].lt(1e-6) & scores["minimum_cash"].ge(0)
    ].copy()
    ...
    return_names = audited.loc[audited["candidate"].isin(return_stable)].sort_values(
        ["total_return", "max_drawdown"], ascending=[False, False]
    )["candidate"].tolist()
    defensive_names = audited.loc[audited["candidate"].isin(defensive_stable)].sort_values(
...
def _anchor_diff(output: Path, anchor_score: Mapping[str, Any]) -> tuple[str | None, float | None]:
    baseline = _baseline_path()
    if baseline is None:
        return None, None
    run_dir = output / "candidates" / "fixed11_gradual" / str(anchor_score["candidate_hash"])[:16]
    actual = pd.read_csv(run_dir / "equity.csv", encoding="utf-8-sig")
    expected = pd.read_csv(baseline, encoding="utf-8-sig")
    aligned = actual[["trade_date", "equity"]].merge(
        expected[["trade_date", "equity"]], on="trade_date", suffixes=("_actual", "_expected")
    )
    if len(aligned) != len(actual) or len(aligned) != len(expected):
        return str(baseline), math.inf
    return str(baseline), float((aligned["equity_actual"] - aligned["equity_expected"]).abs().max())
...
def _walkforward_runner(inputs: ExperimentInputs, target_hashes: Mapping[str, str], initial_cash: float,
                        costs: CostModel):
    def runner(*, candidate: ExperimentCandidate, start: pd.Timestamp, end: pd.Timestamp,
        ...
        result = run_candidate(period, candidate, config, costs=costs)
        score = _score_result(result, _candidate_target_hash(candidate, target_hashes), "period")
...
def run_stages(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if "core" in requested or any(stage in requested for stage in ("routes", "walkforward", "stress")):
        ...
        for index, candidate in enumerate(_core_candidates(), start=1):
            score = _run_full_candidate(candidate, inputs, target_hashes, config, output, costs, args.resume)
            rows.append(score)
        ...
        baseline_source, baseline_diff = _anchor_diff(output, anchor)
        manifest.update({"anchor_source": baseline_source,
                         "anchor_max_abs_equity_diff": baseline_diff,
    ...
    if any(stage in requested for stage in ("routes", "walkforward", "stress")):
        ...
        if "routes" in requested or args.stage in ("walkforward", "stress"):
            ...
            write_csv(rejected, output / "rejected_candidates.csv")
        ...
        if any(stage in requested for stage in ("walkforward", "stress")):
            ...
            wf = run_walk_forward(universe, folds, _walkforward_runner(inputs, target_hashes,
                                                                       args.initial_cash, costs))
        ...
        if "stress" in requested:
            ...
            if len(return_policy) == 5:
                return_policy["combined_max_drawdown"] = _combined_drawdown(return_policy["total_return"])
                by_name = {item.name: item for item in universe}
                runner_2x = _walkforward_runner(inputs, target_hashes, args.initial_cash,
                                                scale_cost_model(costs, 2.0))
                cost_rows = []
                for fold, row in zip(folds, return_policy.itertuples(index=False), strict=True):
                    for series, candidate in (("candidate", by_name[row.candidate]),
                                              ("anchor", ExperimentCandidate.anchor())):
                        score = runner_2x(candidate=candidate, start=fold.test_start,
                                          end=fold.test_end, phase="test", fold=fold.name)
                        cost_rows.append({"fold": fold.name, "series": series,
                                          "total_return": score["total_return"], "cost_multiplier": 2.0})
                cost_frame = pd.DataFrame(cost_rows)
                stress_rows.extend(cost_rows)
                gate_rows.append(asdict(evaluate_route_gates("return", return_policy,
                                                             anchor_tests, cost_2x_scores=cost_frame)))
            else:
                gate_rows.append(asdict(next(item for item in wf.gate_results if item.route == "return")))

            defensive_policy = policy.loc[policy["route"].eq("defensive")]
            if len(defensive_policy) == 5:
                by_name = {item.name: item for item in universe}
                windows = (("2024_q1", "2024-01-01", "2024-03-31"),
                           ("2026_ytd", "2026-01-01", args.end))
                stress_gate_rows = []
                for window, start, end in windows:
                    matching = defensive_policy.loc[defensive_policy["fold"].str.contains(start[:4])]
                    candidate = by_name[matching.iloc[0]["candidate"]] if not matching.empty else by_name[defensive_policy.iloc[-1]["candidate"]]
                    runner = _walkforward_runner(inputs, target_hashes, args.initial_cash, costs)
                    candidate_score = runner(candidate=candidate, start=pd.Timestamp(start), end=pd.Timestamp(end),
                                             phase="stress", fold=window)
                    anchor_score = runner(candidate=ExperimentCandidate.anchor(), start=pd.Timestamp(start),
                                          end=pd.Timestamp(end), phase="stress", fold=window)
                    stress_gate_rows.append({"window": window,
                                             "candidate_max_drawdown": candidate_score["max_drawdown"],
                                             "anchor_max_drawdown": anchor_score["max_drawdown"]})
                stress_frame = pd.DataFrame(stress_gate_rows)
                stress_rows.extend(stress_gate_rows)
                gate_rows.append(asdict(evaluate_route_gates("defensive", defensive_policy,
                                                             anchor_tests, stress_scores=stress_frame)))
            else:
                gate_rows.append(asdict(next(item for item in wf.gate_results if item.route == "defensive")))
            gate_rows.append(asdict(next(item for item in wf.gate_results if item.route == "balanced")))
            ...
            manifest["cost_models"] = {str(multiplier): _jsonable(scale_cost_model(costs, multiplier))
    ...
    if not all_scores.empty:
        manifest["max_account_reconciliation_error"] = float(all_scores["account_reconciliation_error"].max())
        manifest["minimum_cash"] = float(all_scores["minimum_cash"].min())
    manifest["passed"] = bool(
        manifest.get("anchor_source")
        and manifest.get("anchor_max_abs_equity_diff", math.inf) < 1e-6
        and manifest.get("max_account_reconciliation_error", math.inf) < 1e-6
        and manifest.get("minimum_cash", -math.inf) >= 0
        and manifest.get("test_selection_leakage_count", 0) == 0
        and manifest.get("full_sample_candidate_count", len(core_scores)) <= 70
        and manifest.get("normal_test_call_count", 0) <= 45
    )
    write_json(manifest_path, manifest)
    return manifest
