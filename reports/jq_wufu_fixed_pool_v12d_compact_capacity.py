# -*- coding:utf-8 -*-
"""
JoinQuant version: Wufu ETF rotation V12-A sync baseline script.
...
def initialize(context):
    ...
    log.info("WUFU_JQ_FIXED_POOL_V12D_COMPACT_CAPACITY expected_init_cash={} schedule morning=09:40 signal=13:10 execute=13:11 trend=13:40,14:10,14:30 force_buy={} reset=15:10 diagnostics={} execution_buffer={} round_lot={} weak_calendar_version={} ultra_compact={} split_enabled={} split_minutes={} participation={} step_buffer={} weak_indexes=000300.XSHG,399101.XSHE,399006.XSHE,000510.XSHG".format(
        EXPECTED_INIT_CASH, FORCE_BUY_TIME, len(DIAGNOSTIC_DATES), EXECUTION_CASH_BUFFER, ROUND_LOT, WEAK_CALENDAR_VERSION,
        ULTRA_COMPACT_LOGS, CAPACITY_SPLIT_ENABLED, CAPACITY_SPLIT_MINUTES, CAPACITY_PARTICIPATION_RATE, CAPACITY_STEP_BUFFER
    ))
...
def morning_routine(context):
    ...
    if (not ULTRA_COMPACT_LOGS) or today_text in DIAGNOSTIC_DATES or getattr(g, "last_logged_morning_weak", None) != g.is_a_share_weak:
        log.info(
            "WUFU_MORNING_COMPACT date={} weak={} threshold={:.0f} pool={}".format(
        ...
        g.last_logged_morning_weak = g.is_a_share_weak
    ...
    if g.target_etfs_list:
        ...
        if INTRADAY_TIMING_ENABLED:
            ...
            if g.pending_buy_etfs:
                wait_key = today.isoformat() + ":" + ",".join(g.pending_buy_etfs)
                if (not ULTRA_COMPACT_LOGS) or today.isoformat() in DIAGNOSTIC_DATES or getattr(g, "last_pending_state", "") != wait_key:
...
def execute_pending_buy_with_trend(context, force=False):
    ...
    for i, code in enumerate(buy_now):
        ...
        if (not ULTRA_COMPACT_LOGS) or context.current_dt.date().isoformat() in DIAGNOSTIC_DATES:
    ...
    if (not ULTRA_COMPACT_LOGS) or still_pending or context.current_dt.date().isoformat() in DIAGNOSTIC_DATES:
        ...
        g.last_pending_state = context.current_dt.date().isoformat() + ":" + ",".join(g.pending_buy_etfs)
...
def process_capacity_split_orders(context):
    ...
    for code, state in list(orders.items()):
        ...
        risk = _capacity_risk(context, code, {"shares": state.get("initial_shares", 0)})
    ...
    for code in finished:
        ...
        log.info("WUFU_SPLIT_DONE date={} code={} mode={} steps={} final_value={:.2f} actual_value={:.2f} gap_pct={:.4f}".format(
            context.current_dt.strftime("%Y-%m-%d %H:%M:%S"), code, state.get("mode", "NA"),
...
def minute_level_stop_loss(context):
    ...
    for security, position in list(context.portfolio.positions.items()):
        ...
        if price <= cost * FIXED_STOP_LOSS_THRESHOLD:
            ...
            pos_value = _position_value(context, security)
            log.info("WUFU_STOP_LOSS date={} code={} price={:.4f} cost={:.4f} amount={:.0f} closeable={:.0f} position_value={:.2f} loss_pct={:.4f} threshold={:.4f} source=jq".format(
                context.current_dt.strftime("%Y-%m-%d %H:%M:%S"), security, price, cost, position.total_amount, position.closeable_amount, pos_value, loss_pct, FIXED_STOP_LOSS_THRESHOLD
            ))
            try:
                order_target_value(security, 0)
            except Exception as exc:
...
def _log_position(context):
    ...
    log.info("WUFU_DAILY_COMPACT date={} value={:.2f} weak={} pool={} target={} pending={} positions={}".format(
        today, context.portfolio.total_value, g.is_a_share_weak, len(getattr(g, "merged_etf_pool", []) or []),
        ",".join(getattr(g, "target_etfs_list", []) or []), ",".join(getattr(g, "pending_buy_etfs", []) or []),
...
def _current_price(code):
    try:
        ...
        if (not current_data[code].paused) and np.isfinite(price) and price > 0:
            return price
    except Exception:
        pass
    try:
        df = get_price(code, count=1, frequency="1m", fields=["close"], skip_paused=False, fq="pre")
