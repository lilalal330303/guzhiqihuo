...
def initialize(context):
    ...
    log.info("WUFU_JQ_FIXED_POOL_V5 schedule morning=09:40 signal=13:10 execute=13:11 reset=15:10")
...
def execute_routine(context):
    ...
    if g.target_etfs_list:
        buy_value = _rounded_lot_value(g.target_etfs_list[0], context.portfolio.total_value * EXECUTION_CASH_BUFFER)
        if buy_value > 0:
            try:
                order_target_value(g.target_etfs_list[0], buy_value)
            except Exception as exc:
...
def after_trading_end(context):
    log.info("WUFU_POSITION date={} value={:.2f} positions={}".format(
        context.current_dt.date(), context.portfolio.total_value, _position_detail(context)
    ))
...
def _detail_log(message):
    today = None
    try:
        today = str(context.current_dt.date())  # noqa: F821 - JoinQuant global context is unavailable here.
    except Exception:
        pass
    if today is None:
        date_match = None
