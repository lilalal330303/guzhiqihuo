...
def init(context):
    ...
    _register_schedule()
    log.info("WUFU_THS_FAST_V8 auto_weak={} fallback_external={} external_source={} start={} end={} ranges={} dynamic_pool={} full_market_threshold={}".format(
        not USE_EXTERNAL_WEAK_STATES, FALLBACK_EXTERNAL_WEAK_STATES, EXTERNAL_WEAK_SOURCE, EXTERNAL_WEAK_START_DATE, EXTERNAL_WEAK_END_DATE,
        len(EXTERNAL_WEAK_RANGES), USE_DYNAMIC_POOL, USE_FULL_MARKET_THRESHOLD
    ))
    log.info("WUFU_THS_FAST_V8 diagnostics={} execution_buffer={} round_lot={} score_detail_top_n={} weak_indexes={}".format(
        len(DIAGNOSTIC_DATES), EXECUTION_CASH_BUFFER, ROUND_LOT, DEBUG_DETAIL_TOP_N, ",".join(WEAK_INDEXES)
    ))
