...
def calculate_global_etf_threshold(context):
    ...
    daily_totals = []
    for code in codes:
        df = _get_price_1d(context, code, LIQUIDITY_LOOKBACK + 1)
        if len(df) < LIQUIDITY_LOOKBACK + 1:
            continue
        money = _money_series(df).iloc[-(LIQUIDITY_LOOKBACK + 1):-1]
    ...
    return float(np.mean(daily_totals[-LIQUIDITY_LOOKBACK:]) / LIQUIDITY_DIVISOR)
...
def check_weak_state(context):
    ...
    for code in WEAK_INDEXES:
        df = _get_price_1d(context, code, WEAK_MA_LOOKBACK + 2)
        if len(df) < WEAK_MA_LOOKBACK + 1:
            continue
...
def build_dynamic_pool(context):
    ...
    for code, name in get_all_etf_metadata(context):
        ...
        if len(df) < LIQUIDITY_LOOKBACK + 1:
            continue
        avg_amount = float(_money_series(df).iloc[-(LIQUIDITY_LOOKBACK + 1):-1].mean())
...
def select_target(context, data=None):
    ...
    return candidates[0]["code"] if candidates else DEFENSIVE_ETF


def score_symbol(context, code, is_weak, data=None):
    df = _get_price_1d(context, code, LOOKBACK_DAYS + 1)
    ...
    if is_weak and p <= float(pd.Series(closes).tail(MA_LOOKBACK).mean()) * MA_THRESHOLD:
        return None
    if not pass_volume_filter(df, p):
        return None
    if not pass_loss_filter(closes):
        return None
    return {"code": code, "score": score, "annualized": annualized, "r2": r2}
...
def pass_volume_filter(df, current_price):
    if len(df) < VOLUME_LOOKBACK + 1:
        return False
    volumes = df["volume"].astype(float).values
    current_volume = volumes[-1]
    trailing = volumes[-(VOLUME_LOOKBACK + 1):-1]
