"""Iron ore CTA V1.4 aggressive risk tier.

This standalone JoinQuant script keeps the V1.1/V1.3 strict trend signal and
safe execution flow. It uses the C-tier risk budget and adds a 1.25x position
boost only when trend quality is strong and volatility is not elevated.
...
def calculate_drawdown_multiplier(current_value, high_water):
    """Return the aggressive-tier new-entry multiplier from drawdown."""
...
def calculate_trend_quality_multiplier(
    ...
    """Return 1.25 only for strong trend with non-elevated volatility."""
...
def calculate_risk_scaled_amount(
    ...
    """Apply drawdown and trend quality multipliers after base sizing."""
    try:
...
def get_signal_snapshot(signal_date):
    ...
    previous_slow = float(
        pd.Series(closes[-params["trend_days"] - params["slope_days"]:-params["slope_days"]]).mean()
    )
...
def close_position(context, code, direction):
    ...
    if remaining == 0 or is_order_fully_filled(order):
        log.info("V1.4 close code=%s direction=%s amount=%s", code, direction, amount)
        return remaining == 0
    log.info("V1.4 close not complete code=%s remaining=%s", code, remaining)
    return False
...
def open_position(context, code, direction, snapshot):
    ...
    if price is None:
        log.info("V1.4 no valid contract price, skip entry code=%s", code)
        return False
    ...
    g.trend_multiplier = calculate_trend_quality_multiplier(
        snapshot["close"],
        snapshot["ma_fast"],
        snapshot["ma_slow"],
        snapshot["slow_slope"],
    ...
    g.risk_multiplier = g.drawdown_multiplier * g.trend_multiplier
    ...
    if amount <= 0:
        log.info(
            "V1.4 risk budget does not allow new entry code=%s multiplier=%.2f",
            code,
            g.risk_multiplier,
        )
        return False
    ...
    if not is_order_fully_filled(order):
        log.info("V1.4 entry not fully filled code=%s amount=%s", code, amount)
    else:
        log.info(
            "V1.4 entry code=%s direction=%s amount=%s vol=%.4f dd=%.2f trend=%.2f",
            code,
            direction,
            amount,
            snapshot["realized_vol"],
...
def trade_open(context):
    ...
    if snapshot is None:
        log.info("V1.4 insufficient signal data date=%s", signal_date)
        return

    target_contract = get_target_contract(signal_date)
    if not target_contract:
        log.info("V1.4 no eligible iron ore contract date=%s", signal_date)
        return
    ...
    if g.cooldown > 0:
        g.cooldown -= 1
        log.info("V1.4 cooldown after exit, remaining=%s", g.cooldown)
        return
