# -*- coding:utf-8 -*-
"""
JoinQuant version: WUFU_QIXING_FUSION_V2_DUAL_SLOT.

Purpose:
- Keep parameters aligned with the local V10 recent-grid best configuration.
...
WEAK_CONFIRM_DAYS = 2
QIXING_PROFIT_PROTECTION_ENABLED = True
QIXING_PROFIT_PROTECTION_DRAWDOWN = 0.05
DEBUG_DETAIL_TOP_N = 10
DEBUG_DETAIL_SAMPLE_N = 20
EXPECTED_INIT_CASH = 1000000
...
FORCE_BUY_TIME = "14:40"
FIXED_STOP_LOSS_ENABLED = True
FIXED_STOP_LOSS_THRESHOLD = 0.94
STOP_LOSS_WINDOWS = (("13:01", "14:56"),)
...
def initialize(context):
    ...
    g.dual_slot_enabled = DUAL_SLOT_ENABLED
    g.wufu_slot_weight = WUFU_SLOT_WEIGHT
    g.qixing_slot_weight = QIXING_SLOT_WEIGHT
    ...
    g.top10_log = ""
    g.split_orders = {}
    g.last_logged_target = None
    g.last_logged_weak = None
    g.pending_weak_state = None
    g.pending_weak_count = 0
    g.qixing_high_water = {}

    run_daily(morning_routine, time="09:40")
    run_daily(signal_routine, time="13:10")
    run_daily(execute_routine, time="13:11")
    run_daily(process_capacity_split_orders, time="every_bar")
    ...
    log.info("WUFU_QIXING_FUSION_V2_DUAL_SLOT expected_init_cash={} schedule morning=09:40 signal=13:10 execute=13:11 trend=13:40,14:10,14:30 force_buy={} reset=15:10 diagnostics={} execution_buffer={} round_lot={} weak_calendar_version={} ultra_compact={} split_enabled={} split_minutes={} participation={} qixing_enabled={} qixing_pool={} qixing_bonus={} dual_slot={} wufu_weight={} qixing_weight={} weak_confirm_days={} stop_loss_threshold={} weak_indexes=000300.XSHG,399101.XSHE,399006.XSHE,000510.XSHG".format(
        EXPECTED_INIT_CASH, FORCE_BUY_TIME, len(DIAGNOSTIC_DATES), EXECUTION_CASH_BUFFER, ROUND_LOT, WEAK_CALENDAR_VERSION,
        ULTRA_COMPACT_LOGS, CAPACITY_SPLIT_ENABLED, CAPACITY_SPLIT_MINUTES, CAPACITY_PARTICIPATION_RATE,
        g.qixing_enhancement_enabled, len(g.qixing_etf_pool), g.qixing_preferred_pool_bonus,
        g.dual_slot_enabled, g.wufu_slot_weight, g.qixing_slot_weight, WEAK_CONFIRM_DAYS, FIXED_STOP_LOSS_THRESHOLD
    ))
    log.info("WUFU_INTRADAY_CONFIG enabled={} trend_lookback_minutes={} trend_slope_threshold={} stop_loss_enabled={} stop_loss_threshold={} stop_loss_windows={}".format(
        INTRADAY_TIMING_ENABLED, TREND_LOOKBACK_MINUTES, TREND_SLOPE_THRESHOLD, FIXED_STOP_LOSS_ENABLED, FIXED_STOP_LOSS_THRESHOLD, STOP_LOSS_WINDOWS
    ))
...
def morning_routine(context):
    ...
    if g.is_a_share_weak:
        ...
        qixing_pool = g.qixing_etf_pool if g.qixing_enhancement_enabled else []
        g.merged_etf_pool = list(dict.fromkeys(g.filtered_fixed_pool + g.dynamic_etf_pool + qixing_pool))
    log.info(
...
def signal_routine(context):
    ...
    if g.last_signal_date == today:
        return
    if getattr(g, "dual_slot_enabled", False):
        slot_targets, target_weights = select_dual_slot_targets(context)
        g.slot_targets = slot_targets
        g.target_weights = target_weights
        g.target_etfs_list = list(target_weights.keys())
    else:
        target = select_target(context)
        g.slot_targets = {"wufu": target, "qixing": target}
        g.target_weights = {target: 1.0} if target else {}
        g.target_etfs_list = [target] if target else []
    g.last_signal_date = today
    target_text = ",".join(g.target_etfs_list)
    weight_text = ",".join(["{}:{:.4f}".format(code, weight) for code, weight in g.target_weights.items()])
    slot_text = "wufu={},qixing={}".format(g.slot_targets.get("wufu", ""), g.slot_targets.get("qixing", ""))
    if (not ULTRA_COMPACT_LOGS) or g.last_logged_target != target_text or today.isoformat() in DIAGNOSTIC_DATES:
        log.info(
            "WUFU_SIGNAL source=run_daily now={} signal_date={} target={} weights={} slots={} top10={}".format(
                context.current_dt.strftime("%Y-%m-%d %H:%M:%S"),
                today,
                target_text,
                weight_text,
    ...
    g.last_logged_target = target_text
    ...
    if g.target_etfs_list:
        ...
        if INTRADAY_TIMING_ENABLED:
            ...
            if g.pending_buy_etfs:
                if (not ULTRA_COMPACT_LOGS) or today.isoformat() in DIAGNOSTIC_DATES:
                    log.info("WUFU_PENDING_BUY date={} pending={} next_checks=13:40,14:10,14:30 force_buy={}".format(
                        today, ",".join(g.pending_buy_etfs), FORCE_BUY_TIME
                    ))
        else:
            execute_pending_buy_with_trend(context, force=True)
    g.last_execute_date = today
    if (not ULTRA_COMPACT_LOGS) or g.target_etfs_list or today.isoformat() in DIAGNOSTIC_DATES:
        log.info(
...
def execute_pending_buy_with_trend(context, force=False):
    ...
    target_weights = getattr(g, "target_weights", {}) or {}
    equal_weight = 1.0 / max(len(buy_now) + len(still_pending), 1)
    for code in buy_now:
        weight = float(target_weights.get(code, equal_weight) or equal_weight)
        target_value = context.portfolio.total_value * EXECUTION_CASH_BUFFER * weight
        ...
        if plan["order_value"] > 0:
            _place_buy_order_or_split(context, code, plan, target_value, "force" if force else "trend")
        else:
    ...
    if (not ULTRA_COMPACT_LOGS) or buy_now or still_pending or context.current_dt.date().isoformat() in DIAGNOSTIC_DATES:
...
def _place_buy_order_or_split(context, code, plan, target_value, mode="trend"):
    if not CAPACITY_SPLIT_ENABLED:
        try:
            order_target_value(code, plan["order_value"])
        except Exception as exc:
            log.info("WUFU_ORDER_FAIL action=buy code={} mode={} reason={}".format(code, mode, exc))
        return
    risk = _capacity_risk(context, code, plan)
    if not risk["limited"]:
        try:
            order_target_value(code, plan["order_value"])
        except Exception as exc:
    ...
    log.info("WUFU_SPLIT_START date={} code={} mode={} shares={} minute_volume={} cap_shares={} participation={:.2f} splits={} target_value={:.2f}".format(
        context.current_dt.strftime("%Y-%m-%d %H:%M:%S"), code, mode, plan["shares"],
        risk["minute_volume"], risk["cap_shares"], CAPACITY_PARTICIPATION_RATE, splits, plan["order_value"]
    ))
    process_capacity_split_orders(context)


def process_capacity_split_orders(context):
    orders = getattr(g, "split_orders", {}) or {}
    if not orders:
        return
    now_text = context.current_dt.strftime("%Y-%m-%d %H:%M")
    finished = []
    for code, state in list(orders.items()):
        if state.get("done", 0) >= state.get("splits", 1):
            finished.append(code)
            continue
        if state.get("last_time", "") == now_text:
            continue
        ...
        plan = _rounded_lot_plan(code, step_value)
        log.info("WUFU_SPLIT_STEP date={} code={} step={}/{} target_value={:.2f} rounded_value={:.2f} price={:.4f} shares={}".format(
            context.current_dt.strftime("%Y-%m-%d %H:%M:%S"), code, next_done, splits,
            step_value, plan["order_value"], plan["price"], plan["shares"]
        ))
        if plan["order_value"] > 0:
            try:
                order_target_value(code, plan["order_value"])
            except Exception as exc:
                log.info("WUFU_ORDER_FAIL action=split_buy code={} step={}/{} reason={}".format(code, next_done, splits, exc))
        state["done"] = next_done
        state["last_time"] = now_text
        if next_done >= splits:
            finished.append(code)
    for code in finished:
        orders.pop(code, None)
    g.split_orders = orders


def _capacity_risk(context, code, plan):
    minute_volume = _current_minute_volume(context, code)
    cap_shares = int(minute_volume * CAPACITY_PARTICIPATION_RATE / ROUND_LOT) * ROUND_LOT
    limited = bool(minute_volume > 0 and cap_shares > 0 and plan["shares"] > cap_shares)
...
def minute_level_stop_loss(context):
    ...
    if not _in_time_windows(current_time, STOP_LOSS_WINDOWS):
        return
    ...
    for security, position in list(context.portfolio.positions.items()):
        ...
        if price <= 0 or cost <= 0:
            continue
        if _qixing_profit_protection_triggered(context, security, price, position):
            continue
        if price <= cost * FIXED_STOP_LOSS_THRESHOLD:
            loss_pct = price / cost - 1.0
            log.info("WUFU_STOP_LOSS date={} code={} price={:.4f} cost={:.4f} amount={:.0f} closeable={:.0f} loss_pct={:.4f} threshold={:.4f}".format(
                context.current_dt.strftime("%Y-%m-%d %H:%M:%S"), security, price, cost, position.total_amount, position.closeable_amount, loss_pct, FIXED_STOP_LOSS_THRESHOLD
            ))
            try:
                order_target_value(security, 0)
            except Exception as exc:
...
def _qixing_profit_protection_triggered(context, security, price, position):
    if not QIXING_PROFIT_PROTECTION_ENABLED:
        return False
    qixing_codes = set(getattr(g, "qixing_etf_pool", []) or [])
    slot_targets = getattr(g, "slot_targets", {}) or {}
    if security not in qixing_codes and security != slot_targets.get("qixing"):
        return False
    high_water = getattr(g, "qixing_high_water", {}) or {}
    old_high = float(high_water.get(security, 0.0) or 0.0)
    new_high = max(old_high, float(price))
    high_water[security] = new_high
    g.qixing_high_water = high_water
    if new_high <= 0:
        return False
    drawdown = price / new_high - 1.0
    if drawdown > -QIXING_PROFIT_PROTECTION_DRAWDOWN:
        return False
    log.info("WUFU_QIXING_PROFIT_PROTECT date={} code={} price={:.4f} high={:.4f} amount={:.0f} drawdown={:.4f} threshold={:.4f}".format(
        context.current_dt.strftime("%Y-%m-%d %H:%M:%S"), security, price, new_high, position.total_amount,
        drawdown, QIXING_PROFIT_PROTECTION_DRAWDOWN
    ))
    try:
        order_target_value(security, 0)
        if security in getattr(g, "target_weights", {}):
            g.target_weights.pop(security, None)
        if security in getattr(g, "target_etfs_list", []):
            g.target_etfs_list = [code for code in g.target_etfs_list if code != security]
        return True
    except Exception as exc:
        log.info("WUFU_ORDER_FAIL action=qixing_profit_protect_sell code={} reason={}".format(security, exc))
        return False


def _in_time_windows(current_time, windows):
...
def _log_position(context):
    ...
    if getattr(g, "last_position_log_date", None) == today:
        return
    weight_text = ",".join(["{}:{:.4f}".format(code, weight) for code, weight in (getattr(g, "target_weights", {}) or {}).items()])
    log.info("WUFU_DAILY_COMPACT date={} value={:.2f} weak={} pool={} target={} weights={} pending={} positions={}".format(
        today, context.portfolio.total_value, g.is_a_share_weak, len(getattr(g, "filtered_etf_list", [])),
        ",".join(getattr(g, "target_etfs_list", []) or []), weight_text, ",".join(getattr(g, "pending_buy_etfs", []) or []),
        _position_detail(context)
    ))
...
def check_a_share_weak_period(context):
    ...
    desired_state = g.is_a_share_weak
    if g.is_a_share_weak:
        if weak_days >= g.max_weak_days:
            desired_state = False
    ...
    if desired_state == g.is_a_share_weak:
        g.pending_weak_state = None
        g.pending_weak_count = 0
        if g.is_a_share_weak and weak_condition_met:
            g.weak_start_date = context.current_dt.date()
    else:
        if g.pending_weak_state == desired_state:
            g.pending_weak_count += 1
        else:
            g.pending_weak_state = desired_state
            g.pending_weak_count = 1
        if g.pending_weak_count >= WEAK_CONFIRM_DAYS:
            g.is_a_share_weak = desired_state
            g.pending_weak_state = None
            g.pending_weak_count = 0
            g.weak_start_date = context.current_dt.date() if desired_state else None
    today_text = context.current_dt.date().isoformat()
    if (not ULTRA_COMPACT_LOGS) or g.last_logged_weak != g.is_a_share_weak or today_text in WEAK_DEBUG_DATES:
        ...
        g.last_logged_weak = g.is_a_share_weak
    _weak_detail_log("WUFU_WEAK_DETAIL date={} before={} after={} desired={} pending_state={} pending_count={} confirm_days={} below={} above={} weak_start={} weak_days={} detail={}".format(
        context.current_dt.date(), before, g.is_a_share_weak, desired_state, g.pending_weak_state, g.pending_weak_count,
        WEAK_CONFIRM_DAYS, below_count, above_count, g.weak_start_date, weak_days, "|".join(details)
    ))
...
def select_dual_slot_targets(context):
    wufu_pool = g.merged_etf_pool if g.merged_etf_pool else (g.global_etf_pool if g.is_a_share_weak else g.fixed_etf_pool)
    qixing_pool = list(dict.fromkeys(getattr(g, "qixing_etf_pool", []) or []))
    wufu_rows = rank_pool(context, wufu_pool, use_fusion_score=False)
    qixing_rows = rank_pool(context, qixing_pool, use_fusion_score=True)

    wufu_target = wufu_rows[0]["code"] if wufu_rows else g.defensive_etf
    qixing_target = qixing_rows[0]["code"] if qixing_rows else wufu_target
    weights = combine_slot_weights([
        (wufu_target, float(getattr(g, "wufu_slot_weight", WUFU_SLOT_WEIGHT) or 0.0)),
        (qixing_target, float(getattr(g, "qixing_slot_weight", QIXING_SLOT_WEIGHT) or 0.0)),
    ])
    g.top10_log = "wufu=[" + ",".join(["{}:{:.4f}".format(item["code"], item["score"]) for item in wufu_rows[:5]]) + "]"
    g.top10_log += ";qixing=[" + ",".join(["{}:{:.4f}".format(item["code"], item.get("fusion_score", item["score"])) for item in qixing_rows[:5]]) + "]"
    g.score_detail_log = g.top10_log
    _detail_log("WUFU_DUAL_SLOT_DETAIL date={} weak={} wufu_target={} qixing_target={} weights={} wufu_pool={} qixing_pool={}".format(
        context.current_dt.date(), g.is_a_share_weak, wufu_target, qixing_target,
        ",".join(["{}:{:.4f}".format(code, weight) for code, weight in weights.items()]),
        len(wufu_pool), len(qixing_pool)
    ))
    return {"wufu": wufu_target, "qixing": qixing_target}, weights


def rank_pool(context, pool, use_fusion_score=False):
    rows = []
    for code in pool:
        item = score_symbol(code, context)
        if item:
            rows.append(item)
    key_name = "fusion_score" if use_fusion_score else "score"
    rows.sort(key=lambda item: item.get(key_name, item["score"]), reverse=True)
    return rows


def combine_slot_weights(slots):
    weights = {}
    for code, weight in slots:
...
def select_target(context):
    ...
    for code in pool:
        ...
        if item:
            rows.append(item)
    rows.sort(key=lambda item: item.get("fusion_score", item["score"]), reverse=True)
    g.top10_log = ",".join(["{}:{:.4f}".format(item["code"], item.get("fusion_score", item["score"])) for item in rows[:10]])
    g.score_detail_log = "|".join([
        "{}:{:.4f}:{:.4f}:{:.4f}:{:.4f}:{:.4f}:{:.0f}".format(
            item["code"], item.get("fusion_score", item["score"]), item["score"], item.get("qixing_bonus", 0.0),
            item["annualized"], item["r2"], item.get("today_volume", np.nan)
        )
    ...
    reference_item = rows[g.holdings_num - 1] if len(rows) >= g.holdings_num else rows[0]
    reference_score = reference_item.get("fusion_score", reference_item["score"])
    ratio = 1.0 if g.is_a_share_weak else g.score_threshold_ratio
    candidates = [item for item in rows[:10] if item.get("fusion_score", item["score"]) >= reference_score * ratio]
...
def score_symbol(code, context):
    try:
        ...
        if len(closes) >= 4 and np.min(closes[-3:] / closes[-4:-1]) < g.loss:
            return None
        qixing_bonus = 0.0
        if getattr(g, "qixing_enhancement_enabled", False) and code in getattr(g, "qixing_etf_pool", []):
            qixing_bonus = float(getattr(g, "qixing_preferred_pool_bonus", 0.0) or 0.0)
        fusion_score = score + qixing_bonus
        return {
            "code": code,
            "score": score,
            "qixing_bonus": qixing_bonus,
...
def _current_minute_volume(context, code):
    try:
        df = get_price(
            code,
            end_date=context.current_dt,
