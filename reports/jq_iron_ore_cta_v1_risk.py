"""铁矿石 CTA V1.1：保留 V1 趋势框架，加入波动率仓位和安全换月。

使用方法：把本文件完整复制到聚宽策略编辑器。
默认 ALLOW_SHORT=False，只做多/空仓，便于和用户确认的 V1 做公平比较。
将 ALLOW_SHORT 改为 True 后，才启用下跌趋势做空。
"""
...
def classify_v1_signal(
    ...
    """Return 1 bullish, -1 bearish, or 0 neutral after confirmation."""
...
def calculate_atr(bars, window=20):
    """Simple-average true range using high, low, and previous close."""
...
def calculate_vol_scaled_amount(total_value, available_cash, price, realized_vol, params):
    """Size contracts using volatility, leverage, and margin budgets."""
...
def select_near_contract(futures, signal_date, roll_days_before_expiry=8):
    """Select the nearest eligible iron ore contract at a point-in-time date."""
...
def can_open_replacement(old_amount, close_filled, remaining_amount):
    """A replacement is allowed only after the old actual exposure is flat."""
...
def close_position(context, code, direction):
    ...
    if remaining == 0 or is_order_fully_filled(order):
        log.info("V1.1：平仓 code=%s direction=%s amount=%s", code, direction, amount)
        return remaining == 0
    log.info("V1.1：平仓未完成 code=%s remaining=%s", code, remaining)
    return False
...
def open_position(context, code, direction, snapshot):
    ...
    if price is None:
        log.info("V1.1：无有效合约价格，跳过开仓 code=%s", code)
        return False
    ...
    if amount <= 0:
        log.info("V1.1：波动率/保证金预算不足，跳过开仓 code=%s", code)
        return False
    side = "long" if direction > 0 else "short"
    order = order_target(code, amount, side=side)
    if not is_order_fully_filled(order):
        log.info("V1.1：开仓订单未完全成交 code=%s amount=%s", code, amount)
    else:
        log.info(
            "V1.1：开仓 code=%s direction=%s amount=%s vol=%.4f",
            code,
            direction,
            amount,
            snapshot["realized_vol"],
...
def trade_open(context):
    ...
    if snapshot is None:
        log.info("V1.1：信号数据不足，跳过交易 date=%s", signal_date)
        return

    target_contract = get_target_contract(signal_date)
    if not target_contract:
        log.info("V1.1：无满足交割边界的铁矿石合约 date=%s", signal_date)
        return
    ...
    if g.cooldown > 0:
        g.cooldown -= 1
        log.info("V1.1：换月/退出冷却中，剩余=%s", g.cooldown)
        return
