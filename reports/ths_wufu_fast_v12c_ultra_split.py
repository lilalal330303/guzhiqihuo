...
def init(context):
    ...
    _register_schedule()
    log.info("WUFU_THS_FAST_V12C_ULTRA_SPLIT init_cash={} auto_weak={} fallback_external={} weak_calendar_version={} external_source={} start={} end={} ranges={} dynamic_pool={} full_market_threshold={} ultra_compact={} split_enabled={} split_minutes={} participation={}".format(
        INIT_CASH,
        ...
        len(EXTERNAL_WEAK_RANGES), USE_DYNAMIC_POOL, USE_FULL_MARKET_THRESHOLD, ULTRA_COMPACT_LOGS, CAPACITY_SPLIT_ENABLED, CAPACITY_SPLIT_MINUTES, CAPACITY_PARTICIPATION_RATE
    ))
    log.info("WUFU_THS_FAST_V12C_ULTRA_SPLIT diagnostics={} execution_buffer={} round_lot={} score_detail_top_n={} weak_min_valid={} weak_indexes={} a300_probe_indexes={} compact_logs=True".format(
...
def execute_callback(context, data=None, source="run_daily"):
    ...
    if (not ULTRA_COMPACT_LOGS) or context.wufu_target or today in DIAGNOSTIC_DATES:
...
def execute_pending_buy_with_trend(context, data=None, force=False):
    ...
    if (not ULTRA_COMPACT_LOGS) or buy_now or still_pending or get_datetime().strftime("%Y-%m-%d") in DIAGNOSTIC_DATES:
...
def process_capacity_split_orders(context, data=None):
    ...
    for code, state in list(orders.items()):
        ...
        step_value = float(state.get("final_value", 0.0)) * next_done / splits
        plan = _rounded_lot_plan(context, code, step_value, data)
        log.info("WUFU_SPLIT_STEP date={} code={} step={}/{} target_value={:.2f} rounded_value={:.2f} price={:.4f} shares={}".format(
            now.strftime("%Y-%m-%d %H:%M:%S"), code, next_done, splits, step_value,
            plan["order_value"], plan["price"], plan["shares"]
        ))
        if plan["order_value"] > 0:
            _order_target_value(context, code, plan["order_value"])
...
def minute_level_stop_loss(context, data=None):
    ...
    for code in list(_positions(context)):
        ...
        if price <= cost * FIXED_STOP_LOSS_THRESHOLD:
            log.info("WUFU_STOP_LOSS date={} code={} price={:.4f} cost={:.4f} amount={:.0f} loss_pct={:.4f} threshold={:.4f}".format(
                now.strftime("%Y-%m-%d %H:%M:%S"), code, price, cost, amount, price / cost - 1.0, FIXED_STOP_LOSS_THRESHOLD
            ))
