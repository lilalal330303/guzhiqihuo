...
WEAK_INDEX_ALIASES = {
    "000510.SH": ["000510.CSI", "000510.XSHG", "000510.SH", "000510"],
}
...
def init(context):
    ...
    _register_schedule()
    log.info("WUFU_THS_FAST_V5 external_weak={} source={} start={} end={} ranges={} dynamic_pool={} full_market_threshold={}".format(
        USE_EXTERNAL_WEAK_STATES, EXTERNAL_WEAK_SOURCE, EXTERNAL_WEAK_START_DATE, EXTERNAL_WEAK_END_DATE,
        len(EXTERNAL_WEAK_RANGES), USE_DYNAMIC_POOL, USE_FULL_MARKET_THRESHOLD
    ))
    log.info("WUFU_THS_FAST_V7 diagnostics={} execution_buffer={} round_lot={} score_detail_top_n={}".format(
        len(DIAGNOSTIC_DATES), EXECUTION_CASH_BUFFER, ROUND_LOT, DEBUG_DETAIL_TOP_N
    ))
...
def execute_callback(context, data=None, source="run_daily"):
    ...
    if context.wufu_target:
        ...
        log.info("WUFU_ORDER_PLAN date={} code={} account_value={:.2f} buffered_value={:.2f} price={:.4f} shares={} order_value={:.2f}".format(
...
def _external_weak_state(today):
    if not USE_EXTERNAL_WEAK_STATES:
        return None
...
def _rounded_lot_plan(context, code, value, data=None):
    ...
    if not np.isfinite(price) or price <= 0:
        return {"price": 0.0, "shares": 0, "order_value": 0.0}
    shares = int(float(value) / price / ROUND_LOT) * ROUND_LOT
    return {"price": float(price), "shares": int(shares), "order_value": max(0.0, shares * price)}
