...
def calculate_global_etf_threshold(context):
    ...
    _detail_log("WUFU_THRESHOLD_DETAIL date={} universe={} valid={} amount_codes={} fallback_codes={} totals={} threshold={:.0f} source=ths_formula".format(
...
def check_weak_state(context):
    ...
    if is_weak:
        ...
        result = False
    _detail_log("WUFU_WEAK_DETAIL date={} before={} after={} below={} above={} weak_start={} weak_days={} detail={}".format(
...
def score_symbol(context, code, is_weak, data=None):
    ...
    if len(df) < LOOKBACK_DAYS:
        return None
    p, today_volume = _current_price_volume(context, code, data)
...
def _ensure_daily_amount_cache(context, codes):
    ...
    if missing:
        ...
        log.info("WUFU_FAST_CACHE date={} requested={} fetched={} total_cached={}".format(
...
def _fetch_amount_histories(context, codes, count):
    frames = _get_price_1d_many(context, codes, count)
    output = {}
    today = get_datetime().strftime("%Y-%m-%d")
...
def _get_price_1d_many(context, codes, count):
    if FAST_BATCH_PRICE_ENABLED:
        try:
            df = get_price(codes, None, get_datetime(), "1d", ["open", "high", "low", "close", "volume", "amount"], bar_count=count)
            ...
            pass
        try:
            df = get_price(codes, None, get_datetime(), "1d", ["open", "high", "low", "close", "volume"], bar_count=count)
            frames = _split_price_frame(codes, df)
            if frames:
                return frames
        except Exception:
...
def _current_price_volume(context, code, data=None):
    c = _resolve(context, code)
    price = np.nan
    volume = np.nan
    try:
