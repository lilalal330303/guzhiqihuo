...
def initialize(context):
    ...
    log.info("WUFU_JQ_FIXED_POOL_V7 schedule morning=09:40 signal=13:10 execute=13:11 reset=15:10 diagnostics={} execution_buffer={} round_lot={}".format(
        len(DIAGNOSTIC_DATES), EXECUTION_CASH_BUFFER, ROUND_LOT
    ))
...
def execute_routine(context):
    ...
    if g.target_etfs_list:
        plan = _rounded_lot_plan(g.target_etfs_list[0], context.portfolio.total_value * EXECUTION_CASH_BUFFER)
        log.info("WUFU_ORDER_PLAN date={} code={} account_value={:.2f} buffered_value={:.2f} price={:.4f} shares={} order_value={:.2f}".format(
            today, g.target_etfs_list[0], context.portfolio.total_value,
            context.portfolio.total_value * EXECUTION_CASH_BUFFER, plan["price"], plan["shares"], plan["order_value"]
        ))
        if plan["order_value"] > 0:
            try:
                order_target_value(g.target_etfs_list[0], plan["order_value"])
            except Exception as exc:
                log.info("WUFU_ORDER_FAIL action=buy code={} reason={}".format(g.target_etfs_list[0], exc))
        else:
            log.info("WUFU_ORDER_FAIL action=buy code={} reason=rounded_value_zero".format(g.target_etfs_list[0]))
