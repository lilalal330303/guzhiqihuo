...
def execute_callback(context, data=None, source="run_daily"):
    ...
    if context.wufu_target:
        buy_value = _rounded_lot_value(context, context.wufu_target[0], _account_value(context) * EXECUTION_CASH_BUFFER, data)
        if buy_value > 0:
            _order_target_value(context, context.wufu_target[0], buy_value)
        else:
