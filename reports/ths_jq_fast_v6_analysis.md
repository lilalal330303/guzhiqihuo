...
- 目标匹配率为 `0.9847619047619047`，Top10 平均重合 `6.093333333333334`。
- 弱市状态匹配率为 `1.0`，弱市不一致 `0` 天。
- 同花顺成交额阈值 / 聚宽成交额阈值的中位数为 `0.9932244542539431`。
- 同花顺每日请求 ETF 中位数 `None`，聚宽全市场 ETF universe 中位数 `114.0`。
- 同花顺收盘资金从 `998496.53` 到 `69094453.25`，区间收益 `68.19849110542226`，最大回撤 `-0.23671921298598497`。
