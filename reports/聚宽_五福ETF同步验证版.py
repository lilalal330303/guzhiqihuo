...
def check_a_share_weak_period(context):
    ...
    if g.is_a_share_weak:
        weak_days = len(get_trade_days(start_date=g.weak_start_date, end_date=context.current_dt.date())) if g.weak_start_date else 0
...
def select_target(context):
    ...
    return candidates[0]["code"] if candidates else g.defensive_etf
