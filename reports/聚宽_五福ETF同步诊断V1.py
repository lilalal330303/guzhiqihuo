...
def initialize(context):
    ...
    log.info("WUFU_JQ_INIT schedule morning=09:40 signal=13:10 execute=13:11 reset=15:10")
...
def execute_routine(context):
    ...
    if g.target_etfs_list:
        order_target_value(g.target_etfs_list[0], context.portfolio.total_value)
...
def calculate_global_etf_threshold(context):
    try:
        ...
        if not etf_list:
            g.avg_etf_money_threshold = g.liquidity_fallback
            log.info("WUFU_THRESHOLD_DETAIL date={} universe=0 valid=0 totals= threshold={:.0f} source=fallback_empty_universe".format(context.current_dt.date(), g.avg_etf_money_threshold))
            return
        ...
        if len(trade_days) < g.liquidity_lookback:
            g.avg_etf_money_threshold = g.liquidity_fallback
            log.info("WUFU_THRESHOLD_DETAIL date={} universe={} valid=0 totals= threshold={:.0f} source=fallback_insufficient_history".format(context.current_dt.date(), len(etf_list), g.avg_etf_money_threshold))
            return
        df = get_price(
            security=etf_list,
        ...
        if df is None or df.empty:
            g.avg_etf_money_threshold = g.liquidity_fallback
            log.info("WUFU_THRESHOLD_DETAIL date={} universe={} valid=0 totals= threshold={:.0f} source=fallback_empty_price".format(context.current_dt.date(), len(etf_list), g.avg_etf_money_threshold))
            return
        ...
        log.info("WUFU_THRESHOLD_DETAIL date={} universe={} valid={} totals={} threshold={:.0f} source={}".format(
            context.current_dt.date(), len(etf_list), int(df["code"].nunique()) if "code" in df.columns else len(etf_list),
            _fmt_float_list(daily_totals.tail(g.liquidity_lookback).tolist()), g.avg_etf_money_threshold, source
        ))
    except Exception as exc:
        log.warning("WUFU_THRESHOLD failed {}; fallback".format(exc))
        g.avg_etf_money_threshold = g.liquidity_fallback
        log.info("WUFU_THRESHOLD_DETAIL date={} universe=0 valid=0 totals= threshold={:.0f} source=fallback_exception".format(context.current_dt.date(), g.avg_etf_money_threshold))
...
def select_target(context):
    ...
    if not rows:
        log.info("WUFU_SCORE_DETAIL date={} weak={} pool={} passed=0 target={} top10=".format(context.current_dt.date(), g.is_a_share_weak, len(pool), g.defensive_etf))
