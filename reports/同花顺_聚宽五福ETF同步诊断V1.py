...
def morning_callback(context, data=None):
    ...
    context.wufu_dynamic_pool = [] if context.wufu_is_weak else build_dynamic_pool(context)
...
def calculate_global_etf_threshold(context):
    codes = get_all_etf_codes(context)
    daily_totals = []
    valid_codes = 0
    amount_codes = 0
    fallback_codes = 0
    for code in codes:
        df = _previous_daily_rows(_get_price_1d(context, code, LIQUIDITY_LOOKBACK + 2), get_datetime().strftime("%Y-%m-%d"))
        if len(df) < LIQUIDITY_LOOKBACK:
            continue
        valid_codes += 1
        has_amount = "amount" in df.columns and pd.to_numeric(df["amount"], errors="coerce").fillna(0).sum() > 0
        amount_codes += 1 if has_amount else 0
        fallback_codes += 0 if has_amount else 1
        money = _money_series(df).iloc[-LIQUIDITY_LOOKBACK:]
        for i, value in enumerate(money.values):
            if len(daily_totals) <= i:
                daily_totals.append(0.0)
            if np.isfinite(value):
                daily_totals[i] += float(value)
    if len(daily_totals) < LIQUIDITY_LOOKBACK:
        log.info("WUFU_THRESHOLD_DETAIL date={} universe={} valid={} amount_codes={} fallback_codes={} totals={} threshold={:.0f} source=fallback".format(
    ...
    log.info("WUFU_THRESHOLD_DETAIL date={} universe={} valid={} amount_codes={} fallback_codes={} totals={} threshold={:.0f} source=ths_formula".format(
...
def check_weak_state(context):
    ...
    if is_weak:
        ...
        result = False
    log.info("WUFU_WEAK_DETAIL date={} before={} after={} below={} above={} weak_start={} weak_days={} detail={}".format(
...
def build_dynamic_pool(context):
    rows = []
    for code, name in get_all_etf_metadata(context):
        ...
        if len(df) < LIQUIDITY_LOOKBACK:
            continue
        avg_amount = float(_money_series(df).iloc[-LIQUIDITY_LOOKBACK:].mean())
        if np.isfinite(avg_amount) and avg_amount > context.wufu_liquidity_threshold:
            rows.append({"code": code, "key": key, "amount": avg_amount})
    if not rows:
        log.info("WUFU_POOL_DETAIL date={} dynamic_candidates=0 dynamic_pool=".format(get_datetime().strftime("%Y-%m-%d")))
        return []
    ...
    log.info("WUFU_POOL_DETAIL date={} dynamic_candidates={} dynamic_groups={} dynamic_pool={}".format(
...
def select_target(context, data=None):
    rows = []
    is_weak = getattr(context, "wufu_is_weak", False)
    for code in getattr(context, "wufu_pool", []):
        try:
            item = score_symbol(context, code, is_weak, data)
            if item:
                rows.append(item)
        except Exception:
            pass
    ...
    if not rows:
        log.info("WUFU_SCORE_DETAIL date={} pool={} passed=0 top10=".format(get_datetime().strftime("%Y-%m-%d"), len(getattr(context, "wufu_pool", []))))
    ...
    log.info("WUFU_SCORE_DETAIL date={} weak={} pool={} passed={} target={} top10={}".format(
        get_datetime().strftime("%Y-%m-%d"), is_weak, len(getattr(context, "wufu_pool", [])), len(rows), target, getattr(context, "wufu_score_detail", "")
    ))
    return target


def score_symbol(context, code, is_weak, data=None):
    df = _previous_daily_rows(_get_price_1d(context, code, LOOKBACK_DAYS + 2), get_datetime().strftime("%Y-%m-%d"))
    if len(df) < LOOKBACK_DAYS:
        return None
    p = _current_price(context, code, data)
    ...
    if is_weak and p <= float(pd.Series(closes).tail(MA_LOOKBACK).mean()) * MA_THRESHOLD:
        return None
    today_volume = _current_volume(context, code, data)
...
def get_all_etf_metadata(context):
    try:
        ...
        pass
    return [(code, code) for code in FIXED_ETF_POOL]
