...
WEAK_CALENDAR_VERSION = "joinquant_v4_ranges_20200102_20260706_v10_a1000_probe"
...
def init(context):
    ...
    _register_schedule()
    log.info("WUFU_THS_FAST_V10_A1000 auto_weak={} fallback_external={} weak_calendar_version={} external_source={} start={} end={} ranges={} dynamic_pool={} full_market_threshold={}".format(
    ...
    log.info("WUFU_THS_FAST_V10_A1000 diagnostics={} execution_buffer={} round_lot={} score_detail_top_n={} weak_min_valid={} weak_indexes={}".format(
...
def execute_callback(context, data=None, source="run_daily"):
    ...
    if context.wufu_target:
        plan = _rounded_lot_plan(context, context.wufu_target[0], _account_value(context) * EXECUTION_CASH_BUFFER, data)
        log.info("WUFU_ORDER_PLAN date={} code={} account_value={:.2f} buffered_value={:.2f} price={:.4f} shares={} order_value={:.2f} estimated_cost={:.2f} residual_cash={:.2f} round_lot={}".format(
            today, context.wufu_target[0], _account_value(context),
            _account_value(context) * EXECUTION_CASH_BUFFER, plan["price"], plan["shares"], plan["order_value"],
            plan["estimated_cost"], plan["residual_cash"], ROUND_LOT
        ))
        if plan["order_value"] > 0:
            _order_target_value(context, context.wufu_target[0], plan["order_value"])
        else:
            log.info("WUFU_ORDER_FAIL action=buy code={} reason=rounded_value_zero".format(context.wufu_target[0]))
